...
export function FocusRail({
  ...
  return (
    <div
      className={cn(
        "group relative flex h-[600px] w-full flex-col overflow-hidden bg-[#0a0a0c] text-[#f4f4f5] outline-none select-none overflow-x-hidden",
        className
      )}
          ...
          {visibleIndices.map((offset) => {
            ...
            return (
              <motion.div
                key={absIndex}
                className={cn(
                  "absolute aspect-[3/4] w-[260px] md:w-[300px] rounded-2xl border-t border-[rgba(255,255,255,0.07)] bg-[#111116] shadow-2xl transition-shadow duration-300",
                  isCenter ? "z-20 shadow-[rgba(124,92,252,0.2)]" : "z-10"
                )}
                initial={false}
                animate={{
                  x: xOffset,
        ...
        <div className="mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row pointer-events-auto">
          <div className="flex flex-1 flex-col items-center text-center md:items-start md:text-left h-32 justify-center">
            <AnimatePresence mode="wait">
              ...
              >
                {activeItem.meta && (
                  <span className="text-xs font-medium uppercase tracking-wider text-[#7c5cfc]">
                    {activeItem.meta}
                  </span>
                )}
                <h2 className="text-3xl font-bold tracking-tight md:text-4xl text-[#f4f4f5]">
                  {activeItem.title}
                </h2>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 rounded-full bg-[#111116]/80 p-1 ring-1 ring-[rgba(255,255,255,0.07)] backdrop-blur-md">
              <button
                onClick={handlePrev}
                className="rounded-full p-3 text-[#6b6b7a] transition hover:bg-[rgba(255,255,255,0.05)] hover:text-[#f4f4f5] active:scale-95"
                aria-label="Previous"
              >
                <ChevronLeft className="h-5 w-5" />
              </button>
              <span className="min-w-[40px] text-center text-xs font-mono text-[#6b6b7a]">
                {activeIndex + 1} / {count}
              </span>
              <button
                onClick={handleNext}
            ...
            {activeItem.href && (
              <Link
                ...
                className="group flex items-center gap-2 rounded-full bg-[#f4f4f5] px-5 py-3 text-sm font-semibold text-[#0a0a0c] transition-transform hover:scale-105 active:scale-95"
              >
                Explore
