...
export default function MethodologySection() {
  ...
  useGSAP(() => {
    ...
    scrollTriggerRef.current = ScrollTrigger.create({
      ...
      start: "top top",
      // ~0.85 viewport of scroll per transition. Advancing to the next
      // feature is one short flick, and the final snap (snapPoints[last])
      // lands at progress = 1.0 so the section unpins immediately — no
      // "stuck after Collaborate Seamlessly" dead zone.
      end: () => `+=${segments * 85}%`,
      pin: true,
      // Small scrub only — panels swap at discrete thresholds, so a large
      // scrub is pure dead-time. 0.3 keeps it smooth but responsive.
      scrub: 0.3,
      onUpdate: (self) => {
        // Round progress to the nearest panel so the active panel and the
        // snap target always agree — no lingering between snaps.
        const page = Math.min(
          pages.length,
